...
export async function renderView(container, teamId, query = {}) {
  ...
  const notice = query.expired
    ? h("div.card.pad-s", { style: { borderColor: "var(--goldlight)", background: "#FFF9E8", marginBottom: "14px" } }, h("b", "열람 세션이 만료되었거나 유효하지 않습니다."), " 핀을 다시 입력하세요 (세션은 24시간 유지).")
    : loggedIn
  ...
  const card = (t) => {
    ...
    const form = h("form.stack", { style: { display: open ? "" : "none", marginTop: "10px", gap: "8px" }, onsubmit: async (e) => {
      ...
      try {
        ...
        toast(`${r.category.name} 열람을 시작합니다 (24시간)`);
  ...
  mount(container, shell(cfg, h("div", { style: { maxWidth: "980px", margin: "40px auto 0" } },
    h("div.hero", { style: { padding: "0 0 22px" } }, h("div.eyebrow", "Public View"), h("h1", "핀으로 열람"), h("p.sub", "관리자가 공개로 설정한 팀은 토큰이나 가입 없이 핀만 입력하면 진행 상황을 볼 수 있습니다. 읽기 전용이며 열람 세션은 24시간 뒤 만료됩니다. 핀은 연구책임자나 리드에게 확인하세요.")),
    notice,
